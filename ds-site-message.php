...
class DS_SITE_MESSAGE {
	function startup() {
		...
		if ( is_admin() ) {
			...
			if (
				 ( isset( $dssm_settings['content']['status'] ) && $dssm_settings['content']['status'] && !current_user_can( 'edit_plugins' ) )
				|| isset( $_GET['dssm-preview'] )
			) {
	...
	*/
	function update_settings() {
		if( DSSM_VERSION !== get_option( 'dssm-version' ) )
			$this->activate();
	}
